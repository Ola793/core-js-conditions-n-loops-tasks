...
function convertToRomanNumerals(num) {
  let romanNum = '';
  const romanDec = Math.floor(num / 10);
  const romanUnit = num % 10;
  let decCount = '';
  let unitsCount = '';

  if (num >= 10) {
    for (let i = 1; i <= romanDec; i += 1) {
      decCount += 'X';
    }
  }
  ...
  if (romanUnit) {
    if (romanUnit <= 3) {
      ...
      unitsCount = 'V';
    } else if (romanUnit >= 6 && romanUnit <= 8) {
      unitsCount = 'V';

      for (let i = 6; i <= romanUnit; i += 1) {
        unitsCount += 'I';
      }
    } else if (romanUnit === 9) {
      unitsCount = 'IX';
...
function rotateMatrix(matrix) {
  ...
  for (let i = 0; i < matrixLength; i += 1) {
    for (let j = 0; j < matrixLength / 2; j += 1) {
      const temp = matrix[i][j];
      rotatedMatrix[i][j] = matrix[i][matrixLength - j - 1];
      rotatedMatrix[i][matrixLength - j - 1] = temp;
    }
  }
...
function sortByAsc(arr) {
  ...
  for (let i = 1; i < arr.length; i += 1) {
    ...
    let j;
    for (j = i - 1; j >= 0 && arr[j] > currentEl; j -= 1) {
      resArr[j + 1] = arr[j];
    }
    resArr[j + 1] = currentEl;
  }
...
function shuffleChar(str, iterations) {
  ...
  for (let i = 0; i < iterations; i += 1) {
    if (seen.has(currentStr)) {
      const cycleStart = seen.get(currentStr);
      const cycleLength = i - cycleStart;
      const remainingIterations = (iterations - i) % cycleLength;
      return shuffleChar(currentStr, remainingIterations);
    }
    seen.set(currentStr, i);
